...
const VAWChartDetails = ({
  ...
  return (
    <div className="mb3 lg-flex serif fs-18">
      <div className="mb2 sm-mb0 sm-mr7 flex-auto">
        {!nibrsDetails ? (
          <p className="m0" style={{ maxWidth: 400 }}>
            ...
            {pluralize('were', actual)} {highlight(fmt(actual))} reported{' '}
            {pluralize(noun, actual)} of {crimeDisplay}. There{' '}
            {pluralize('were', cleared)} {highlight(fmt(cleared))} cleared{' '}
            {crimeDisplay} {pluralize(noun, cleared)}. Crimes are not
            necessarily cleared in the year they occur. {compSentence}
          </p>
        ) : (
          <p className="m0" style={{ maxWidth: 400 }}>
            In <span id="selected-year-text">{highlight(year)}</span>, there{' '}
            {pluralize('were', value)} {highlight(fmt(value))} reported{' '}
            {pluralize(noun, value)} of {crimeDisplay}.
          </p>
        )}
      </div>
              ...
              <tr key={i}>
                <td className="pr2 sm-pr3 fs-12 nowrap truncate align-bottom col-8">
                  ...
                  <Term id={mapReported(k)} size="sm">
                    {k === 'value'
                      ? startCase(mapReported('offenses'))
...
export default VAWChartDetails
