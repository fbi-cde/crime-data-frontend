...
const ExplorerIntroState = ({
  ...
  if (untilUcr) {
    totalAgencies = untilUcr.active_agency_count
    coveredAgencies = untilUcr.published_agency_count
  }
    ...
    return (
      <div>
        {!isArson ? (
          <div>
            <p className="serif">
              {crimeTerm} rates for {placeName} are derived from {reportTerms}{' '}
              reports voluntarily submitted to the FBI.
            </p>
            <p className="serif">
              In {highlight(until)}
              , the FBI <EstimatedTerm /> crime statistics for {placeName} based
              on data received from {highlight(formatNum(coveredAgencies))} law
              enforcement agencies out of {highlight(formatNum(totalAgencies))}{' '}
          ...
          <div>
            ...
            <p className="serif">
              In {until}, {formatNum(coveredAgencies)} {placeName} law
              enforcement agencies voluntarily reported data to the FBI. The
              charts below feature unestimated data.
            </p>
          </div>
        )}
      </div>
