...
const lookupStateByName = (stateData, name) => {
  ...
  Object.keys(stateData).forEach(data => {
    if (
      slugify(lowerCase(stateData[data].state_name)) ===
...
const lookupRegionByName = (regionData, name) => {
  ...
  Object.keys(regionData).forEach(data => {
    if (
      lowerCase(regionData[data].region_name) === lowerCase(name) ||
      slugify(lowerCase(regionData[data].region_name)) === lowerCase(name)
    ) {
      n = regionData[data]
    }
  })
...
const stateFromAbbr = (stateData, abbr) =>
  stateData.find(d => d.state_abbr === abbr.toLowerCase())

const stateFromName = (stateData, name) =>
  stateData.find(d => d.state_name === name.toLowerCase())
