...
export const asrFilterByYear = (asrData, since, until) => {
  const places = Object.keys(asrData).map(place => {
    const filtered = asrData[place].filter(y => {
...
export const asrCombinePlaces = (asrData, offenses = []) => {
  const places = Object.keys(asrData)
  const years = asrData[places[0]].map(y => y.data_year)
  return years.map(year =>
    Object.assign(
      { year },
      ...places.map(place => {
        const o = {}
        const yearData = asrData[place].find(y => y.data_year === year)
