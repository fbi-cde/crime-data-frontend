...
const validateFilter = filter => {
  if (filter.crime && !isValidCrime(filter.crime))
    filter.crime = defaults.crime;
  ...
  if (filter.since && filter.until && filter.until - filter.since < 10)
    filter.since = filter.until - 10;
  return filter;
};
